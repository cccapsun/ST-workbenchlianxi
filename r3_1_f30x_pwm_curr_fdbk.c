...
__weak void R3_1_Init( PWMC_R3_1_Handle_t * pHandle )
{
  ...
  {
    /* disable IT and flags in case of LL driver usage 在LL驱动程序使用下禁用IT和标志
    ...
    if ( TIMx == TIM1 )
    {
      /* TIM1 Counter Clock stopped when the core is halted 当内核停止时，TIM1 计数器时钟停止*/
      LL_DBGMCU_APB2_GRP1_FreezePeriph( LL_DBGMCU_APB2_GRP1_TIM1_STOP );
    }
#ifdef TIM8    
    else
    {
      /* TIM8 Counter Clock stopped when the core is halted 当内核停止时，TIM8 计数器时钟停止 */
      LL_DBGMCU_APB2_GRP1_FreezePeriph( LL_DBGMCU_APB2_GRP1_TIM8_STOP );
    }
#endif
    /* Over current protection phase A  A相过电流保护 */
    if ( COMP_OCPAx != NULL )
    {
      /* Inverting input*/
